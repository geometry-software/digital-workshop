...
import { appendId } from './repository.utils'
...
export class RepositoryService {
   ...
   */
  getDocumentById = <T>(collection, id: string): Observable<T> => this.angularFirestore.doc<T>(collection + '/' + id).valueChanges()
  ...
  getCollectionSize = <S>(collectionName: string, status: S): Observable<number> =>
    from(getCountFromServer(query(collection(this.angularFirestore.firestore, collectionName), where('status', '==', status)))).pipe(
  ...
  getNextPage = <T, S, V>(collection: string, order: OrderRequest, size: number, status: S, value: V): Observable<T[]> =>
    this.angularFirestore
      .collection<T>(collection, (query) => query.orderBy(order.key, order.value).where('status', '==', status).startAfter(value).limit(size))
      .snapshotChanges()
      .pipe(map(appendId<T[]>))
  ...
  getPreviousPage = <T, S, V>(collection: string, order: OrderRequest, size: number, status: S, value: V): Observable<T[]> =>
    this.angularFirestore
      .collection<T>(collection, (query) => query.orderBy(order.key, order.value).where('status', '==', status).endBefore(value).limitToLast(size))
      .snapshotChanges()
      .pipe(map(appendId<T[]>))
   ...
   */
  createDocument = <T>(collection: string, item: T): Observable<DocumentReference> => from(this.angularFirestore.collection(collection).add(item))
  ...
  updateDocument = <T>(collection: string, item: T, id: string): Observable<void> =>
    from(this.angularFirestore.collection(collection).doc(id).update(item))
}
